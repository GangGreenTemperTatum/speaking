...
class UpdateGoal(Action):
    ...
    async def run(self, state: "State") -> str:
        ...
        if user_input.lower() != "y":
            self.goal = (
                input("What is the real goal? (empty for keep existing): ") or self.goal
            )
...
class SaveMemory(Action):
    ...
    def xml_example(cls) -> str:
        return SaveMemory(
            key="my-note", content="Lots of custom data\nKeep this for later."
        ).to_pretty_xml()
...
async def agent_loop(
    ...
    async def parse_actions(chat: rg.Chat) -> t.Optional[rg.Chat]:
        ...
        if len(parsed) > state.max_actions:
            logger.warning(
                f"Model provided more actions than allows {len(parsed)} > {state.max_actions}"
            )
...
def cli(
    ...
    state = State(
        client=client, max_actions=max_actions, base_chat=base_chat, goals=[first_goal]
    )
